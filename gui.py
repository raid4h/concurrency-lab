...
class ProcessTab(tk.Frame):
    ...
    def _run(self, use_lock):
        ...
        def worker():
            ...
            def forward():
                # Bridges the multiprocessing.Queue (used by separate processes) into
                # the regular queue.Queue that the GUI safely polls on the main thread.
...
if __name__ == "__main__":
    multiprocessing.freeze_support()  # required for multiprocessing to work correctly on Windows
    init_db()
    root = tk.Tk()
